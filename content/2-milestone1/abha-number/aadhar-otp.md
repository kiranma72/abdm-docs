...
5. The Aadhaar number provided by the user **must not be stored** by the HMIS / LMIS system to ensure it is compliant with Aadhaar related regulations. 
...
|1.11| {{% badge style="blue" %}}Mandatory & Optional{{% /badge %}} Either of the test cases CRT_ABHA_114 or CRT_ABHA_115 is mandatory for Governement Optional for Private. View and Download ABHA details. (If integrators  is not  generating ABHA card)  (CRT_ABHA_115)| If Integrator is not generating ABHA card.|1. If Integrator is not generating ABHA card then They print the mentioned information on their own card. a. ABHA Number (Mandatory). b. ABHA Address|
