...
> Note: Certain HIPs (and HIUs) only accept the public key in the X.509 format. The `x509PublicKey` field in the output of the `generate-key-material` command can be used to obtain the X.509 public key. However any of the two public key formats can be used with the `encrypt` and `decrypt` commands of Fidelius CLI.
